...
public class ArticleDaoImpl extends BaseDaoImpl<Article> implements ArticleDao {
    ...
    public List<Article> findAllByCategoryIdAndTagIdOrderBy(long categoryId, long tagId, String order, int max, int offset) {
        ...
        if (categoryId >= 0 && tagId >= 0) {
            hql = "select DISTINCT(ac.article) from ArticleCategory ac,ArticleTag at where at.article.id=ac.article.id and at.tag.id=" + tagId + " and ac.category.id=" + order + " order by ac.article." + order + " desc";
        } else if (categoryId < 0 && tagId >= 0) {
            hql = "select DISTINCT(at.article) from ArticleTag at where at.article.id=ac.article.id and at.tag.id=" + tagId + " order by at.article." + order + " desc";
        } else if (categoryId >= 0 && tagId < 0) {
            hql = " from ArticleCategory ac where  ac.category.id=" + order + " order by ac.article." + order + " desc";
        } else {
